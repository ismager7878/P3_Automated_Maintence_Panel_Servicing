...
class PreprocessingNode(Node):
    ...
    def transform_depth(self, img, show=False):
        ...
        try:
            ...
            self.get_logger().error(f"Error in preprocessing node, contour detection: {ex}")
            return adjustedImg, np.eye(3)
        ...
        dst_points = np.array([[0,0],[684,0],[684,861],[0,861]],dtype="float32")
        transform_matrix = cv.getPerspectiveTransform(corner_points, dst_points)
        warped_img = cv.warpPerspective(adjustedImg, transform_matrix, (861, 684))
