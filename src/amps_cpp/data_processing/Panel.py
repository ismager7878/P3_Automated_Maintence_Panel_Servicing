    ...
    def every_iou(self,type,bbox_get_list,bbox_det_list):
        ...
        for x, row in enumerate(iou_holder):
            ...
            for i, btn in enumerate(row):
                if(self.filename == "datasets/auto_aligned_dataset/button_pose2/img3_0"):
                    print("kiger her hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh")
        ...
        for i in range(len(iou_score)):
            for j in range(i + 1, len(iou_score)):
                ...
                if np.array_equal(iou_score[i].pos, iou_score[j].pos):
                    print("Same position found between index", i, "and", j)
                    print(iou_score[i].type, iou_score[j].type)
                    print(self.filename)
                    print(iou_score[i].type )
                    print(iou_score[j].type)
